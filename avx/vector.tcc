...
///////////////////////////////////////////////////////////////////////////////////////////
//// addition
template<typename T> inline Vector<T> &ALWAYS_INLINE Vector<T>::operator+=(const Vector<T> &x) {
    d.v() = VectorHelper<T>::add(data(), x.data()); return *this;
}
template<typename T> inline Vector<T> ALWAYS_INLINE Vector<T>::operator+(const Vector<T> &x) const {
    return Vector<T>(VectorHelper<T>::add(data(), x.data()));
}
template<> inline Vector<int> &ALWAYS_INLINE Vector<int>::operator+=(const Vector<int> &x) {
    d.x(0) = _mm_add_epi32(d.x(0), x.d.x(0));
    d.x(1) = _mm_add_epi32(d.x(1), x.d.x(1));
    return *this;
}
template<> inline Vector<int> ALWAYS_INLINE Vector<int>::operator+(const Vector<int> &x) const {
    Vector<int> r;
...
template<> inline Vector<unsigned int> ALWAYS_INLINE Vector<unsigned int>::operator+(const Vector<unsigned int> &x) const {
